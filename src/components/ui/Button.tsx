...
export default function Button({ children, variant = 'primary', size = 'md', href, onClick, icon, className, type = 'button' }: ButtonProps) {
  ...
  return <button type={type} onClick={onClick} className={classes}>{icon}{children}</button>;
}
